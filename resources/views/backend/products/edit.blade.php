...
@section('content')
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-header py-3 d-flex justify-content-between">
            <h6 class="m-0 font-weight-bold text-primary">تعديل البيانات {{ $product->name }}</h6>
            <div class="ml-auto">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 col-sm-12 pt-4">
                                ...
                                <input type="text" name="sku" id="sku"
                                    value="{{ old('sku', $product->sku) }}" class="form-control" placeholder="sku..">
                                @error('sku')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 col-sm-12 pt-4">
                                ...
                                <input type="number" name="quantity" id="quantity"
                                    ...
                                    placeholder="-1" data-parsley-range="[-1,1000]"
                                ...
                                <div class="col-md-12 col-sm-12 ">
                                    <label class="col-form-label col-md-12 col-sm-12 ">
                                        <input id="checkIn" name="Quantity_Unlimited" type="checkbox" class="flat"
                                            ...
                                            {{ $product->quantity == null ? 'checked' : '' }}> الكمية غير محدودة
                        ...
                        <div class="row">
                            <div class="col-md-6 col-sm-12 pt-3">
                                ...
                                <input type="text" name="price" id="price"
                                    value="{{ old('price', $product->price) }}" class="form-control" placeholder="price">
                                @error('price')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 com-sm-12 pt-4">
                                ...
                                <div class="form-group">
                                    <input type="text" id="offer_ends" name="offer_ends"
                                        value="{{ old('offer_ends', $product->offer_ends) }}" class="form-control">
                                    @error('offer_ends')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6 col-sm-12 pt-4">
                                ...
                                <input type="number" name="max_order" id="max_order"
                                    value="{{ old('max_order', $product->max_order) }}" min="0"
                                    class="form-control" placeholder="0" data-parsley-range="[-1,1000]"
                                    data-parsley-required-message="هذا الحقل مطلوب." placeholder="0"
                                    data-parsley-range="[-1,1000]"
                                    data-parsley-range-message="قيمة هذا الحقل بين [0,999].">
                                @error('max_order')
                        ...
                        <div class="row">
                            <div class="col-6 pt-4">
                                ...
                                <select name="status" class="form-control">
                                    <option value="1"
                                        {{ old('status', $product->status) == '1' ? 'selected' : null }}>مفعل</option>
                                    <option value="0"
                                        {{ old('status', $product->status) == '0' ? 'selected' : null }}>غير مفعل</option>
                                </select>
                                @error('status')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            ...
                            <div class="col-6 pt-4">
                                ...
                                <select name="featured" class="form-control">
                                    <option value="1"
                                        {{ old('featured', $product->featured) == '1' ? 'selected' : null }}>نعم</option>
                                    <option value="0"
                                        {{ old('featured', $product->featured) == '0' ? 'selected' : null }}>لا</option>
                                </select>
                                @error('featured')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            ...
                            <div class="col-sm-12 col-md-6 pt-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="published_on" name="published_on"
                                        value="{{ old('published_on', \Carbon\Carbon::parse($product->published_on)->Format('Y-m-d')) }}"
                                        class="form-control">
                                    @error('published_on')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                            ...
                            <div class="col-sm-12 col-md-6 pt-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="published_on_time" name="published_on_time"
                                        value="{{ old('published_on_time', \Carbon\Carbon::parse($product->published_on)->Format('h:i A')) }}"
                                        class="form-control">
                        ...
                        <div class="row ">
                            ...
                            <div class="col-md-6 col-sm-12 pt-4">
                                ...
                                <select name="tags[]" class="form-control select2" multiple="multiple">
                                    @forelse ($tags as $tag)
                                        <option value="{{ $tag->id }}"
                                            {{ in_array($tag->id, old('tags', $product->tags->pluck('id')->toArray())) ? 'selected' : null }}>
                                            {{ $tag->name }}</option>
                                    @empty
                                    @endforelse
                                </select>
